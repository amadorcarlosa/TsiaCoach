...
import type { BaseTenAction, BaseTenResult } from '~/composables/useBaseTenScene'
const props = defineProps<{ x: number; y: number; choices: readonly { label: string; action: BaseTenAction; result: BaseTenResult }[] }>()
const emit = defineEmits<{ action: [action: BaseTenAction]; close: [] }>()
...
<template>
  <Teleport to="body">
    <div ref="element" class="base-ten-menu" role="menu" aria-label="Base-ten actions" :style="{ left: `${left}px`, top: `${top}px` }" @keydown="keydown">
      <button
...
<style scoped>
.base-ten-menu { position: fixed; z-index: 1000; width: 260px; max-height: calc(100vh - 16px); overflow: auto; padding: 8px; border: 1px solid var(--mt-border); border-radius: var(--radius-md); background: var(--mt-bg-elevated); color: var(--mt-text); box-shadow: 0 8px 30px #0003; }
