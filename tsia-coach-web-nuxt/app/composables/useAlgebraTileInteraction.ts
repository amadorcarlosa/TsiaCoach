...
import type { AlgebraTileAction, useAlgebraTileScene } from './useAlgebraTileScene'

export function useAlgebraTileInteraction(options: {
  scene: ReturnType<typeof useAlgebraTileScene>; viewport: Ref<HTMLElement | null>; disabled: () => boolean; cancelVersion: () => number
}) {
  const { scene } = options
  ...
  const move = shallowRef<{ id: string; ids: string[]; origin: Point; source: typeof scene.tiles.value } | null>(null)
  ...
  function beginMove(id: string) {
    ...
    const tile = scene.tiles.value.find(tile => tile.id === id)
    if (!tile) return false
    select(id, 'preserve')
    move.value = { id, ids: [...scene.selection.value], origin: { ...tile.anchor }, source: scene.tiles.value }
    return true
  }
  ...
  function settle(id: string, position: Point) {
    const session = move.value
    if (session?.id === id && !blocked.value && session.source === scene.tiles.value) scene.apply(session.ids, { type: 'move', delta: { x: position.x - session.origin.x, y: position.y - session.origin.y } })
    delta.value = { x: 0, y: 0 }
  }
  ...
  function apply(action: AlgebraTileAction) {
  ...
  function boardPoint(event: PointerEvent): Point | null {
    ...
    return point ? { x: Math.max(0, Math.min(scene.columns, point.x)), y: Math.max(0, Math.min(scene.rows, point.y)) } : null
  }
  function pointerDown(event: PointerEvent) {
    if (options.disabled() || event.button !== 0 || !event.isPrimary || !(event.target instanceof Element) || event.target.closest('[data-algebra-tile-id]') || !event.target.closest('[data-grid-world]')) return
  ...
  function pointerMove(event: PointerEvent) {
    ...
    const hits = scene.tiles.value.filter(tile => overlaps(rectangle.value!, { ...tile.anchor, width: algebraTileDimensions(tile).width, depth: algebraTileDimensions(tile).depth })).map(tile => tile.id)
